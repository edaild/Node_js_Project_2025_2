...
-- 1. 새로운 아이탬 추가
INSERT INTO items(NAME, DESCRIPTION, VALUE)VALUE
('용검', ' 용 기본무기' , 20)

-- 2. 특정 프레이어의 인벤토리에 새 아이탬 추가
INSERT INTO inventories(player_id, item_id, quantity) VALUES
(2,4,2);
...
INSERT INTO quests(title, DESCRIPTION, reward_exp, reward_item_id) VALUES
('어둠의 성','어둠의 성으로 가세요', 800, 1);

-- 2. 특정 플레이어의 모든 퀘스트 상태 초회
SELECT q.title, pq.status FROM player_quests pq
JOIN quests q ON pq.player_id = q.quest_id WHERE pq.player_id = 1
